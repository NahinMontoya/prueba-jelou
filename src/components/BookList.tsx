...
const BookList: React.FC = () => {
  ...
  const arregloParsed = arregloEnJson ? JSON.parse(arregloEnJson) : [];
  ...
  const handleFavorites = (bookSelected: any) => {
    const isExistFavorite = booksFavorites.find(
      (book: any) =>
        book.book.title.toLocaleLowerCase() ===
        bookSelected.book.title.toLocaleLowerCase()
    );
  ...
  useEffect(() => {
    if (selectedCategory) {
      const filteredBooks = books.filter(
        (book: any) => book.book.genre === selectedCategory
      );
      setBooksFilterd(filteredBooks);
    } else {
      setBooksFilterd(books);
    }
    ...
    <div className='tw-bg-gray-200 tw-p-4'>
      ...
      <div className='grid grid-cols-4 gap-4'>
        {booksFiltered.map((book: any) => {
          const isFiltered = booksFavorites.find(
            (bookSelected: any) =>
              bookSelected.book?.title?.toLocaleLowerCase() ===
              book.book.title.toLocaleLowerCase()
          );
      ...
      {modalFavoritos && (
        <ModalFavoritos
          isOpen={modalFavoritos}
          setIsOpen={handleViewModal}
          books={booksFavorites}
          handleFavorites={handleFavorites}
        />
      )}
    </div>
